...
void main() {
    ...
    // NORMALS + REFRACTION PHYSIQUE
    vec3 N = vNormal + waterNormal(vUv);
    vec3 refractDir = refract(-normalize(vViewDir), N, 1.0 / 1.333);
    vec2 distort = refractDir.xz * 0.01;
    vec2 rcoord = reflect(normalize(vViewDir), N).xz;
    float aberration = 0.002;

    vec2 distortedUV = screenUV + distort;

    // fond de scène — UV distordus
    ...
    // refraction + aberration chromatique
    vec3 refraction;
    ...
    // profondeur locale surface → fond en Y
    ...
    // ASSEMBLAGE
    vec3 waterColor = vec3(0.1, 0.09, 1.0);
